...
void AutoClicker4() {
    while (autoRunning) {
        ClickAtPosition(977, 222);
        std::this_thread::sleep_for(std::chrono::milliseconds(90));
        ClickAtPosition(1143, 736);
        std::this_thread::sleep_for(std::chrono::milliseconds(20));
        ClickAtPosition(1717, 770);
        std::this_thread::sleep_for(std::chrono::milliseconds(20));
        ClickAtPosition(1722, 838);
        std::this_thread::sleep_for(std::chrono::milliseconds(20));
        ClickAtPosition(1180, 792);
        std::this_thread::sleep_for(std::chrono::milliseconds(200));
    }
}
...
void AutoClicker5() {
    while (autoRunning) {
        ClickAtPosition(1040, 215);
        std::this_thread::sleep_for(std::chrono::milliseconds(90));
        ClickAtPosition(1200, 690);
        std::this_thread::sleep_for(std::chrono::milliseconds(20));
        ClickAtPosition(1700, 720);
        std::this_thread::sleep_for(std::chrono::milliseconds(20));
        ClickAtPosition(1700, 835);
        std::this_thread::sleep_for(std::chrono::milliseconds(20));
        ClickAtPosition(1200, 795);
        std::this_thread::sleep_for(std::chrono::milliseconds(200));
    }
}
...
int main() {
    std::cout << "블로그 플러스 매크로 2.2.4ver 입니다.\n"
        << "제작자 : 준벗\n\n"
        << "해상도 1920*1080, 2560*1440만 지원합니다.\n"
        << "A~F는 디스플레이 100%, 웹 100% 기준입니다..\n"
